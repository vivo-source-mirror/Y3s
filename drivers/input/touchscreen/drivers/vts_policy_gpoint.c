...
{
	struct vts_device *vtsdev = local_policy_data.vtsdev;
	int ret = 0;
	ret = vts_call_func_sync(vtsdev, int, vts_check_status_sliently); 
	if (ret) {
		VTE("fail to check status!\n");
	}
	queue_delayed_work(vtsdev->gpoint_wq, &local_policy_data.dwork, msecs_to_jiffies(5000));
}
...
static int vts_policy_mode_exit_prepare(struct vts_device *vtsdev, enum vts_run_mode mode)
{
	...
	if (mode == VTS_ST_NORMAL) {
		ret = cancel_delayed_work(&local_policy_data.dwork);  
		if (ret < 0) {
...
*/
	local_policy_data.vtsdev = vtsdev;
	INIT_DELAYED_WORK(&local_policy_data.dwork, vts_policy_regular_slience_dump_delay_work);
	return 0;
}

struct vts_policy policy_gpoint_dump = {
	.name = "ghost_point_dump",
