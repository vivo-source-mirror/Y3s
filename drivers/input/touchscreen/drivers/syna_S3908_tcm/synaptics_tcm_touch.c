...
static int touch_parse_report(void)
{
	...
	while (idx < config_size) {
		...
		case TOUCH_GESTURE_COORDINATE:
			...
			for (j = 0; j < bits/32; j++) {
				int x = 0, y = 0;
				x = (touch_data->gesture_coordinate[j * 4] | touch_data->gesture_coordinate[j * 4 + 1] << 8) * display_x / dimention_x;
				y = (touch_data->gesture_coordinate[j * 4+ 2] | touch_data->gesture_coordinate[j * 4 + 3] << 8) * display_y / dimention_y;
				if ((x == 0 && y == 0) || (x > display_x && y > display_y)) {
					x = 0xffff;
					y = 0xffff;
				}
		...
		case TOUCH_REPORT_BROKEN_LINE_FLAG:
			...
			vts_report_channel_comp_data(tx_num, rx_num, tx_data, rx_data);
