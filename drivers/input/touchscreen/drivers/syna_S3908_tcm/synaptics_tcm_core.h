...
struct syna_tcm_hcd {
	...
	ktime_t ktime;
	struct semaphore syna_irq_sem;
	struct work_struct syna_reset_work;
	struct semaphore syna_dev_sem;
	struct wakeup_source *syna_pm_wakelock;
	...
	struct task_struct *syna_irq_task;
#ifdef CONFIG_FB
		...
		struct syna_tcm_buffer *buffer, unsigned int size)
{
	...
	if (size > buffer->buf_size) {
		...
		buffer->buf = kmalloc(size, GFP_DMA);
		if (!(buffer->buf)) {
...
static inline int syna_tcm_alloc_mem_dma(struct syna_tcm_hcd *tcm_hcd,
		struct syna_tcm_buffer *buffer, unsigned int size)
{
	if (size > buffer->buf_size) {
		kfree(buffer->buf);
...
extern int syna_module_init(void);
