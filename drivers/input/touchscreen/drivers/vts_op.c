...
//extern int report_channel_comp_data(u8 Txnum, u8 Rxnum ,u16 Txbit, long long Rxbit);
//extern int get_touch_info(int lcmid, u64 fwversion);
...
int vts_report_channel_comp_data(u8 Txnum, u8 Rxnum ,u16 Txbit, long long Rxbit)
{
#ifdef TOUCH_DATA_COLLECT
	call_symbol(report_channel_comp_data,"%d, %d, %d, %llx", Txnum, Rxnum, Txbit, Rxbit);
#endif
	return 0;

}
int vts_report_fw_version(int lcmid, u64 fwversion)
{
#ifdef TOUCH_DATA_COLLECT
	call_symbol(get_touch_info,"%d,%lld",lcmid,fwversion);
#endif
	return 0;
