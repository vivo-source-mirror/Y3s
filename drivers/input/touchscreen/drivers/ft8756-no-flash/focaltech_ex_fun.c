...
static ssize_t fts_debug_write(struct file *filp, const char __user *buff, size_t count, loff_t *ppos)
{
    ...
    if ((count == 0) || (count > FTS_DBUG_BUF_MAX_LEN)) {
        VTE("apk proc wirte count(%d) fail", (int)count);
        return -EINVAL;
    }
	writebuf = ts_data->dbg_write_buf;
	if(NULL == writebuf) {
		VTE("malloc writebuf fail");
		return -EINVAL;
	 }
	 tmp = ts_data->dbg_temp_buf;
	 if(NULL == tmp ) {
		VTE("malloc tmp fail");
		return -EINVAL;
	 }
	
	memset(writebuf, 0 ,FTS_DBUG_BUF_MAX_LEN);
	memset(tmp, 0 ,FTS_DBUG_BUF_MAX_LEN);
...
static ssize_t fts_debug_read(struct file *filp, char __user *buff, size_t count, loff_t *ppos)
{
	...
	buf = ts_data->dbg_read_buf;
	if(NULL == buf){
		VTE("vmalloc buf fail");
		return -EINVAL;
	}
	
	memset(buf, 0, FTS_DBUG_BUF_MAX_LEN);
#if FTS_ESDCHECK_EN
