...
int ilitek_tddi_mp_test_handler_V2(char *apk, bool lcm_on)
{
	int ret = 0;
	...
	if (!idev_V2->chip->open_c_formula ||
		!idev_V2->chip->open_sp_formula) {
		ipio_err("formula is null\n");
		return -1;
	}

	atomic_set(&idev_V2->mp_stat, ENABLE);
	ilitek_plat_irq_disable_V2();
	if (idev_V2->actual_tp_mode != P5_X_FW_TEST_MODE) {
		if (ilitek_tddi_switch_mode_V2(&tp_mode) < 0)
			goto out;
	}

	ret = ilitek_tddi_mp_test_main_V2(apk, lcm_on);
