...
static ssize_t vts_touchpanel_devices_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	...
	list_for_each_entry_safe(vtsdev, tmp, &vtsc->tps, tp_list) {
		...
		count += snprintf(&buf[count], 255, "%s:%d ", vts_name(vtsdev), ic_number);
		i++;
	}
...
static int vts_update_screen_clock_zone(struct vts_screen_clock_zone *dis, struct vts_screen_clock_cmd *src)
{
	 ...
	 if(0 == src->x && 0 == src->y && 0 == src->width  && 0 == src->height)// remove screen clock 
	 	statcnt = 0;
...
static ssize_t vts_service_cmd_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
	...
	struct vts_rejection_data rejection_zone;
	...
	if(strstr(tmp, "vts_rejection_zone")){
		for (type = 0; type <= VTS_TYPE_SECOND; type++) {
			...
			if (vtsdev) {
				...
				if (IS_ERR(vts_get_rejection_zone(&rejection_zone, tmp))) {
					VTE("get rejection zone error\n");
					return -EINVAL;
				}
				VTI("the rejction_zone is type:%d, mode:%d, block:%d, x1:%d, y1:%d, x2:%d, y2:%d\n",rejection_zone.type, rejection_zone.mode, 
					rejection_zone.block, rejection_zone.x1, rejection_zone.y1, rejection_zone.x2, rejection_zone.y2);
				ret = vts_call_ic_ops_sync(vtsdev, set_rejection_zone, &rejection_zone);
				if (ret) {
					VTE("call ic ops set_rejection_zone failed, ret = %d\n", ret);
					return ret;
				}
				return count;
			}
...
static ssize_t vts_fwver_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	...
	list_for_each_entry(vtsdev, &vtsc->tps, tp_list) {
		...
		count += snprintf(buf + strlen(buf), PAGE_SIZE, "%s:0x%llx ", vts_name(vtsdev), version);
	}
	mutex_unlock(&vtsc->lock);
...
static ssize_t vts_module_id_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	...
	list_for_each_entry(vtsdev, &vtsc->tps, tp_list) {
		...
		count += snprintf(&buf[count], PAGE_SIZE - count, "%s:0x%x ", vts_name(vtsdev), vendor);
	}
