...
static int FPPointCnt_GT9897 = 0;
...
static int brl_power_on(struct goodix_ts_core *cd, bool on)
{
	...
	if (regulator_count_voltages(cd->avdd) > 0) {
			ret = regulator_set_voltage(cd->avdd, 3000000,
								3000000);
			ts_info(" avdd regulator set ");
			if (ret) {
				ts_info(" avdd regulator set_vtg:bus_reg vcc_ana failed r=%d", ret);
				regulator_put(cd->avdd);
				return ret;
			}
		}
	if (cd->avdd && on) {
...
int brl_gesture(struct goodix_ts_core *cd, int gesture_type)
{
	...
	cmd.data[0] = 0;
	cmd.data[1] = 0;
	ts_info("enter send brl_gesture");
			...
			struct goodix_fw_version *version)
{
	...
	for (i = 0; i < GOODIX_READ_VERSION_RETRY; i++) {
		...
		//ts_info("read fw version:%*ph", (int)sizeof(buf), buf);
		if (ret) {
...
static void goodix_FPreport(struct goodix_ts_core *data, 
		unsigned char buf)
{

	struct vts_device *vtsdev = data->vtsdev;
	if (0x88 == (buf & 0x88)) {
		if (FPPointCnt_GT9897<= 1)
			FPPointCnt_GT9897++;
	} else { 
		if (FPPointCnt_GT9897 >= 1) {
				...
				u8 *pre_buf, u32 pre_buf_len)
{
	...
	large_status = buffer[2] & 0x10;
	if (0 != large_status) {
		VTI(" Palm Detect, in large status");
		large_press = 1;
		vts_report_event_down(cd->vtsdev, VTS_EVENT_GESTURE_LARGE_AREA_PRESS);
	} else {
		if (large_press) {
			VTI(" Palm release, out large status");
			large_press = 0;
			vts_report_event_up(cd->vtsdev, VTS_EVENT_GESTURE_LARGE_AREA_PRESS);
		}
	}
    ...
    VTD("buffer[0]=0x%2x,FPPointCnt_V2=0x%2x", buffer[0], FPPointCnt_GT9897);
	goodix_FPreport(cd, buffer[0]);
			 ...
			 struct goodix_ts_event *ts_event)
{
	...
	VTD("event_status = 0x%x",event_status);
	...
	if (event_status & GOODIX_REQUEST_EVENT) {
		...
		ts_event->request_code = pre_buf[2];
	}
...
static int brl_set_spi_speed(struct goodix_ts_core *cd, int speed)
{
	struct goodix_bus_interface *bus = cd->bus;
	struct spi_device *spi = to_spi_device(bus->dev);
	spi->max_speed_hz    = speed;
	return 0;
}
#define GOODIX_ENTER_CHARGER_CMD 0x10
#define GOODIX_EXIT_CHARGER_CMD 0x11

static int brl_set_charger(struct goodix_ts_core *cd, int state)
{
  	struct goodix_ts_cmd charger_cmd;
	ts_info("set charge bit state:%d",state);
	if(1 == state)
	charger_cmd.cmd = GOODIX_ENTER_CHARGER_CMD;
	else
	charger_cmd.cmd = GOODIX_EXIT_CHARGER_CMD;	
	charger_cmd.len = 4;
	if (cd->hw_ops->send_cmd(cd, &charger_cmd))
		ts_err("failed send enter charger cmd");
	
	ts_info("set charge bit success:%d",state);
	return 0;
}
static struct goodix_ts_hw_ops brl_hw_ops = {
	...
	.set_spi_speed = brl_set_spi_speed,
	.set_charger = brl_set_charger,
};
