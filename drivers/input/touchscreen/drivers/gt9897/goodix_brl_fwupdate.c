...
static int goodix_parse_firmware(struct firmware_data *fw_data)
{
	...
	ts_info("Fimware VID:i%*ph", 4, fw_summary->fw_vid);
		...
		const char *buf, size_t count)
{
	...
	ret = gt9897_do_fw_update(NULL, mode);
	if (!ret) {
...
static int goodix_fw_update_thread(void *data)
{
	...
	gt9897_ts_blocking_notify(NOTIFY_FWUPDATE_START, NULL);
	...
	if(0)//no need to release firmware-zw add
...
out:
	...
	if (r) {
		...
		gt9897_ts_blocking_notify(NOTIFY_FWUPDATE_FAILED, NULL);
	} else {
		ts_info("fw update success");
		gt9897_ts_blocking_notify(NOTIFY_FWUPDATE_SUCCESS, NULL);
	}
	return r;
}
int goodix_cpy_fw(const struct firmware *src)
{
  goodix_fw_update_ctrl.fw_data.firmware = src;
  return 0;
}
