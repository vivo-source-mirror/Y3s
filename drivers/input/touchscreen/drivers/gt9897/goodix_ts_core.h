...
#define GOODIX_CORE_DRIVER_NAME		"goodix_gt9897"
#define GOODIX_PEN_DRIVER_NAME		"goodix_ts,pen"
#define GOODIX_DRIVER_VERSION		"v0.0.2"
...
struct goodix_ts_hw_ops {
	...
	int (*set_spi_speed)(struct goodix_ts_core *cd, int speed);
	int (*set_charger)(struct goodix_ts_core *cd, int state);
};
...
/* log macro */
#define ts_info			VTI //(fmt, arg...)	pr_info("[GTP-INF][%s:%d] "fmt"\n", __func__, __LINE__, ##arg)
#define	ts_err			VTE//(fmt, arg...)	pr_err("[GTP-ERR][%s:%d] "fmt"\n", __func__, __LINE__, ##arg)
#ifdef CONFIG_GOODIX_DEBUG
#define ts_debug	VTD//pr_info("[GTP-DBG][%s:%d] "fmt"\n", __func__, __LINE__, ##arg)
#else
#define ts_debug	VTD
#endif

#define WRITE_C900_01 1
/*
...
int goodix_gt9897_fw_update_init(struct goodix_ts_core *core_data);
void goodix_gt9897_fw_update_uninit(void);
int gt9897_do_fw_update(struct goodix_ic_config *ic_config, int mode);
int goodix_cpy_fw(const struct firmware * src);
