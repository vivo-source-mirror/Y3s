...
/*Jarvis 2020-7-8 end*/
		...
		{	/*"c" e w m*/
			...
			if (gs_event.gesture_type == 0xcc) {
				/* Jarvis:there have not completed the 
				  x = (gsx_gesture_V2->gesture_buffer_data[1] << 8) | gsx_gesture_V2->gesture_buffer_data[0];
				  y = (gsx_gesture_V2->gesture_buffer_data[3] << 8) | gsx_gesture_V2->gesture_buffer_data[2];
