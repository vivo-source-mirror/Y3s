...
#define GOODIX_TOOLS_NAME		"gtp_tools"//"gtp_tools_gt9897"
...
/*
 * struct gt9897_tools_dev - goodix tools device struct
 ...
 * @mutex: Protect gt9897_tools_dev
 * @in_use: device in use
 */
struct gt9897_tools_dev {
 ...
 */
static int async_read(struct gt9897_tools_dev *dev, void __user *arg)
{
	u8 *databuf = NULL;
	int ret = 0;
	u32 reg_addr, length;
 ...
 */
static int sync_read(struct gt9897_tools_dev *dev, void __user *arg)
{
	int ret = 0;
	u8 i2c_msg_head[I2C_MSG_HEAD_LEN];
 ...
 */
static int async_write(struct gt9897_tools_dev *dev, void __user *arg)
{
	u8 *databuf;
	int ret = 0;
	u32 reg_addr, length;
 ...
 */
static int get_module_info(struct gt9897_tools_dev *dev, void __user *arg)
{
	u8 *databuf = NULL;
	int ret = 0;
	u32 length;
	u8 msg_head[I2C_MSG_HEAD_LEN];
	struct goodix_fw_version *fw_ver = &dev->ts_core->fw_version;
	u8 pid_len = (u8)(sizeof(fw_ver->patch_pid));
	...
	vts_get_lcmid(dev->ts_core->vtsdev, &lcmid);
	...
	databuf[0] = pid_len;
	memcpy(&databuf[1], fw_ver->patch_pid, pid_len);
	...
	//ts_info("databuf:%*ph", info_len, databuf);
...
{
	int ret = 0;
	struct gt9897_tools_dev *dev = filp->private_data;
	struct goodix_ts_core *ts_core;
	...
	case GTP_ESD_ENABLE:
		if (arg == 0)
			gt9897_ts_blocking_notify(NOTIFY_ESD_OFF, NULL);
		else
			gt9897_ts_blocking_notify(NOTIFY_ESD_ON, NULL);
		break;
	case GTP_DEV_RESET:
...
static int goodix_tools_open(struct inode *inode, struct file *filp)
{
	...
	ret = gt9897_register_ext_module_no_wait(&gt9897_tools_dev->module);
	if (ret) {
	...
	gt9897_ts_blocking_notify(NOTIFY_ESD_OFF, NULL);
	filp->private_data = gt9897_tools_dev;
	atomic_set(&gt9897_tools_dev->in_use, 1);
	return 0;
}
...
static int goodix_tools_release(struct inode *inode, struct file *filp)
{
	...
	atomic_set(&gt9897_tools_dev->in_use, 0);
	gt9897_ts_blocking_notify(NOTIFY_ESD_ON, NULL);
	ret = gt9897_unregister_ext_module(&gt9897_tools_dev->module);
	return ret;
}
...
static int goodix_tools_module_irq(struct goodix_ts_core *core_data,
	struct goodix_ext_module *module)
{
	struct gt9897_tools_dev *dev = module->priv_data;
...
static int goodix_tools_module_init(struct goodix_ts_core *core_data,
			struct goodix_ext_module *module)
{
	struct gt9897_tools_dev *tools_dev = module->priv_data;
 ...
 */
int gt9897_tools_init(void)
{
	int ret;

	gt9897_tools_dev = kzalloc(sizeof(struct gt9897_tools_dev), GFP_KERNEL);
	if (gt9897_tools_dev == NULL) {
		ts_err("Memory allco err");
		return -ENOMEM;
	}
	ts_info("gt9897 tools init");
	INIT_LIST_HEAD(&gt9897_tools_dev->head);
	gt9897_tools_dev->ops_mode = 0;
	gt9897_tools_dev->ops_mode |= IRQ_FALG;
	init_waitqueue_head(&gt9897_tools_dev->wq);
	mutex_init(&gt9897_tools_dev->mutex);
	atomic_set(&gt9897_tools_dev->in_use, 0);

	gt9897_tools_dev->module.funcs = &goodix_tools_module_funcs;
	gt9897_tools_dev->module.name = GOODIX_TOOLS_NAME;
	gt9897_tools_dev->module.priv_data = gt9897_tools_dev;
	gt9897_tools_dev->module.priority = EXTMOD_PRIO_DBGTOOL;
...
void gt9897_tools_exit(void)
{
	misc_deregister(&goodix_tools_miscdev);
	kfree(gt9897_tools_dev);
