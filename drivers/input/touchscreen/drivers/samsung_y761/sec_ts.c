...
static int bbk_slsi_set_rejection_zone(struct vts_device *vtsdev, struct vts_rejection_data *data)
{
	int ret = 0;
	struct sec_ts_data *ts = vts_get_drvdata(vtsdev);
	u8 set_data[10];
	...
	if (data->type == 0x100) {
		...
		VTI("set_data_temp = %d, %d, %d, %d, %d", set_data_temp[0], set_data_temp[1], set_data_temp[2], set_data_temp[3], set_data_temp[4]);
