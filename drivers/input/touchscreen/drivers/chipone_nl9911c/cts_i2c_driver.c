    ...
    for (i = 0; i < num; i++) {
        ...
        switch (msgs[i].event) {
            ...
            case CTS_DEVICE_TOUCH_EVENT_STAY:
                contact++;
                vts_report_point_down(cts_data->vtsdev, msgs[i].id, num, x, y, msgs[i].pressure, msgs[i].pressure, false, NULL, 0, kt);
                break;
            case CTS_DEVICE_TOUCH_EVENT_UP:
                ...
                VTI("id = %d, num = %d, x= %d, y = %d",  msgs[i].id, num, x, y);
...
//noise add start
       freq_index = touch_info->add_info & 0x07;
       water_reject_mode = (touch_info->add_info >> 3) & 0x01;
       noise_level      = (touch_info->add_info >> 4) & 0x0f;
//noise add end
