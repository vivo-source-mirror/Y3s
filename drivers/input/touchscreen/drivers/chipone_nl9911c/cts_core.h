...
enum cts_device_fw_reg {
    ...
    CTS_DEVICE_FW_REG_GESTURE_EN_MAP = 0x7020,
    ...
    //noise add start
    CTS_DEVICE_FW_REG_JITTER_FRAME = 0x4700,        //2byte,w
    CTS_DEVICE_FW_REG_JITTER_MODE  = 0x4702,     //1byte,w
    CTS_DEVICE_FW_REG_JITTER_START = 0x4703,        //1byte,w
    CTS_DEVICE_FW_REW_JITTER_END   = 0x4704,        //1byte,w
    CTS_DEVICE_FW_REG_JITTER_READY = 0x4705,        //1byte,rw
    CTS_DEVICE_FW_REG_JITTER_DATA_LEN = 0x4706,     //1byte,r
    CTS_DEVICE_FW_REG_JITTER_FREQ_INDEX = 0x4707,   //1byte,w
    CTS_DEVICE_FW_REG_JITTER_DATA  = 0x4800,        //R
    //noise add end
};
...
enum cts_firmware_cmd {
    ...
    CTS_CMD_SUSPEND_WITH_GESTURE_REPEAT = 0x42,
    ...
    CTS_CMD_ENABLE_IDLE_MODE  = 0xB0,
    CTS_CMD_DISABLE_IDLE_MODE  = 0xB1,
    CTS_CMD_IDLE_COM_EXIT      = 0xB2,
...
#define CTS_CHIP_MAX_GESTURE_TRACE_POINT    (10u)
...
//extern int cts_set_gesture(struct cts_device *cts_dev, u32 enable);
...
// noise add start
extern int cts_set_jitter_frame(struct cts_device *cts_dev, u16 frame);
extern int cts_set_jitter_mode(struct cts_device *cts_dev, u8 mode);
extern int cts_start_jitter(struct cts_device *cts_dev);
extern int cts_stop_jitter(struct cts_device *cts_dev);
extern int cts_get_jitter_data(struct cts_device *cts_dev, void *data);
extern int cts_set_scanfreq(struct cts_device *cts_dev, u8 freq_index);
// noise add end
