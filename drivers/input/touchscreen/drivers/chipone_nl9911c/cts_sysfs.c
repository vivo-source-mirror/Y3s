...
static ssize_t write_firmware_register_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
...
err_post_flash_operation:
    ...
    if (!program_mode) {
        r = cts_enter_normal_mode(cts_dev);
        if (r) {
