...
//        cts_dev->rtdata.slave_addr, cts_dev->rtdata.addr_width * 2, addr, len);
    ...
    //kt_debug_2 = ktime_get();
        ret = cts_plat_spi_read(cts_dev->pdata, cts_dev->rtdata.slave_addr,
                addr_buf, cts_dev->rtdata.addr_width, dst, xfer_len, retry, delay);
    //kt_debug_3 = ktime_get();
    //VTI("spi_test : cts_plat_spi_read kill time %d us",ktime_to_us(ktime_sub(kt_debug_3,kt_debug_2)));
        if (ret) {
            cts_err("Platform i2c read failed %d", ret);
            return ret;
        }
    ...
    //kt_debug_4 = ktime_get();
    //VTI("spi_test : spi while kill time %d us",ktime_to_us(ktime_sub(kt_debug_4,kt_debug_1)));
    return 0;
}
...
const static struct cts_sfctrl icnl9911c_sfctrl = {
    ...
    //.xchg_sram_base = (64 - 1) * 1024,
    //.xchg_sram_size = 1024, /* For non firmware programming */
    .xchg_sram_base = 64 * 1024 - 16,
    .xchg_sram_size = 16, /* For non firmware programming */
    .ops = &cts_sfctrlv2_ops
};
...
static int icnl9911_set_access_ddi_reg(struct cts_device *cts_dev, bool enable)
{
    ...
    if (ret) {
        cts_err("Write password failed %d", ret);
...
{
    int ret;
    ret = cts_fw_reg_readw(cts_dev, CTS_DEVICE_FW_REG_VERSION, version);
...
{
    int ret;
    ret = cts_fw_reg_readb(cts_dev, CTS_DEVICE_FW_REG_DDI_VERSION, version);
...
//noise add 
ret = cts_fw_reg_readb(cts_dev, CTS_DEVICE_FW_REG_JITTER_DATA_LEN, &val);
   /* if (ret || (val != 1 && val != 2)) {
        cts_err("Read firmware jitter data_len failed %d", ret);
        return ret;
    }*/
    fwdata->jitter_data_len = val;
    cts_info("  %-24s: %d", "Jitter data_len", val);
...
//panshan add gesture mode communication
int cts_idle_com_active(struct cts_device *cts_dev)
{
    int ret;
    u8 tmp;

    ret = cts_fw_reg_readb_retry(cts_dev, 0, &tmp, 3, 1);
    if (ret) {
        cts_err("Idle communication enter error");
    }
    return ret;
}

int cts_idle_com_exit(struct cts_device *cts_dev)
{
    int ret;

    ret = cts_send_command(cts_dev, CTS_CMD_IDLE_COM_EXIT);
    if (ret) {
        cts_err("Idle communication exit error ");
    }
    return ret;
}

static int cts_init_fw_area(struct cts_device *cts_dev)
{
    int i;
    u8 buf[3] = {0x72, 0x73, 0x74}; // ascii : rst

    for (i = 0; i < 3; i++) {
        cts_sram_writesb_retry(cts_dev, 0x13ff4, (void *)buf, 3, 3, 1);
    }
    return 0;
}
//
...
//panshan add gesture mode communication
   u16 fwid;
    ...
    /*normal power reset need clear fw status first*/
    if (cts_dev->rtdata.gesture_wakeup_enabled) {
        cts_info("Clear fw status when resume");
        ...
        if (ret) {
            cts_err("Read fwid error");
        }
        cts_info("Read fwid:0x%04x", fwid);
        cts_init_fw_area(cts_dev);
...
#endif /* CONFIG_CTS_I2C_HOST */
//vivo panshan add
	 cts_info("write addr=0x38000 data=0x81000003");
	 ret = cts_hw_reg_writel_retry(cts_dev,0x38000,0x81000003, 3, 1);
	 if (ret) {
	  cts_err("write addr=0x38000 data=0x81000003 failed %d",ret);
	 }
...
int cts_enter_program_mode(struct cts_device *cts_dev)
{
    ...
    do {
        ...
        do {     
            ...
            if (boot_mode != CTS_DEV_BOOT_MODE_SPI_PROGRAM)
            {
                cts_dbg("BOOT_MODE readback %u != I2C/SPI PROMGRAM mode", boot_mode);
                continue;
            }
...
int cts_enter_normal_mode(struct cts_device *cts_dev)
{
    ...
    for(retries = 0; retries < 3; retries++) {
        ret = cts_get_dev_boot_mode(cts_dev, &boot_mode);
        if (ret) {
            cts_err("Get CURR_BOOT_MODE failed %d, try to do reset %s",
                ret, cts_strerror(ret));
            goto reset_device;
        }
...
//noise add start
int cts_set_jitter_frame(struct cts_device *cts_dev, u16 frame)
{
    cts_info("Set jitter frame %u", frame);
    jitter_frame = frame; 
    if (jitter_frame < 3)
        jitter_frame = 3;    
    return 0;
...
int cts_set_jitter_mode(struct cts_device *cts_dev, u8 mode)
{
    cts_info("Set jitter continue %u", mode);
    jitter_mode = mode;
    return 0;
}

int cts_get_jitter_ready(struct cts_device *cts_dev, u8 *ready)
{
    int ret;

    cts_info("Get jitter ready");
    ret = cts_fw_reg_readb_retry(cts_dev, CTS_DEVICE_FW_REG_JITTER_READY,\
        ready, 3, 0);
    if (ret) {
        cts_err("get jitter ready error %d", ret);
    }

    return ret;
}

int cts_clr_jitter_ready(struct cts_device *cts_dev) 
{
    int ret;

    cts_info("Clear jitter ready flag");
    ret = cts_fw_reg_writeb_retry(cts_dev, CTS_DEVICE_FW_REG_JITTER_READY,\
        0, 3, 0);
    if (ret) {
        cts_err("clr jitter ready error %d", ret);
    }

    return ret;
}

int cts_set_scanfreq(struct cts_device *cts_dev, u8 freq_index)
{
    int ret;

    cts_info("Set freq:%u", freq_index);
    ret = cts_fw_reg_writeb_retry(cts_dev, CTS_DEVICE_FW_REG_JITTER_FREQ_INDEX,\
        freq_index, 3, 0);
    if (ret) {
        cts_err("Set scanfreq error %d", ret);
    }
    return ret;
}

int cts_start_jitter(struct cts_device *cts_dev)
{
    int ret;

    cts_info("Start jitter");
    ret = cts_set_work_mode(cts_dev, 1);
    if (ret) {
        cts_err("Set work mode to 1");
        goto err_cts_start_jitter_exit;
    }    
    if (jitter_mode == 0) {
        ret = cts_fw_reg_writew_retry(cts_dev, CTS_DEVICE_FW_REG_JITTER_FRAME,\
            jitter_frame, 3, 0);
        if (ret) {
            cts_err("Write jitter frame");
            goto err_cts_start_jitter_exit;
        }
    }    
    ret = cts_fw_reg_writeb_retry(cts_dev, CTS_DEVICE_FW_REG_JITTER_MODE,\
        jitter_mode, 3, 0);
    if (ret) {
        cts_err("Write jitter mode");
        goto err_cts_start_jitter_exit;
    }
    ret = cts_fw_reg_writeb_retry(cts_dev, CTS_DEVICE_FW_REG_JITTER_START,\
        1, 3, 1);   
    if (ret) {
        cts_err("Start jitter:%d", ret);
        goto err_cts_start_jitter_exit;
    }  
    return 0;
err_cts_start_jitter_exit: 
    ret = cts_set_work_mode(cts_dev, 0);
    if (ret) {
        cts_err("Set work mode to 0");
    }       
    return -1;
}

int cts_stop_jitter(struct cts_device *cts_dev)
{
    int ret;

    cts_info("Stop jitter");
    ret = cts_fw_reg_writeb_retry(cts_dev, CTS_DEVICE_FW_REW_JITTER_END,\
        1, 3, 1);   
    if (ret) {
        cts_err("Stop jitter error:%d", ret);
    }  
    return ret;
}

int cts_get_jitter_data(struct cts_device *cts_dev, void *data)
{
    u32 i, j;
    int ret;
    int ret1;
    ...
    u8 retries = 5;
    u8 *cache_buf;
    u8 data_len = cts_dev->fwdata.jitter_data_len;
    u8 *u8Ptr_s, *u8Ptr_d;
    u16 *u16Ptr_s, *u16Ptr_d;
    u32 ready_time_out;

    cts_info("Get jitter data");
    ready_time_out = jitter_frame * 20;
    if (ready_time_out < 1000)
        ready_time_out = 1000;
    cache_buf = kzalloc(
      (cts_dev->fwdata.rows + 2) * (cts_dev->fwdata.cols + 2) * data_len, GFP_KERNEL);
    if (cache_buf == NULL) {
        cts_err("Get jitter data: malloc error");
        ret = -ENOMEM;
        goto get_jitter_exit_malloc;
    }
    /** - Wait jitter data ready flag set */
    for (i = 0; i < ready_time_out; i++) {
        mdelay(1);
        ret = cts_get_jitter_ready(cts_dev, &ready);
        if (ret) {
            cts_err("Get jitter data ready flag failed %d", ret);
    ...
    do {
        ret = cts_fw_reg_readsb_delay_idle(cts_dev, CTS_DEVICE_FW_REG_JITTER_DATA,
            cache_buf,(cts_dev->fwdata.rows + 2)*(cts_dev->fwdata.cols + 2)*data_len, 500);
        if (ret) {
            cts_err("Read rawdata failed %d", ret);
        }
    } while (--retries > 0 && ret != 0);

    u8Ptr_s  = (u8 *)cache_buf;
    u8Ptr_d = (u8 *)data;
    u16Ptr_s = (u16 *)cache_buf;
    u16Ptr_d = (u16 *)data;

    for (i = 0; i < cts_dev->fwdata.cols; i++) {
        for (j = 0; j< cts_dev->fwdata.rows; j++) {
            if (data_len == 1) {
                u8Ptr_d[i * cts_dev->fwdata.rows + j] = 
                    u8Ptr_s[(j + 1) * (cts_dev->fwdata.cols + 2) + 1 \
                        + cts_dev->fwdata.cols - 1 - i];
            }
            else {
                u16Ptr_d[i * cts_dev->fwdata.rows + j] = 
                    u16Ptr_s[(j + 1) * (cts_dev->fwdata.cols + 2) + 1\
                        + cts_dev->fwdata.cols - 1 - i];            
            }
        }    
    }
    if (cts_clr_jitter_ready(cts_dev)) {
        cts_err("Clear jitter data ready flag failed");
        ret = -ENODEV;
    }
get_jitter_exit:
    kfree(cache_buf);    
get_jitter_exit_malloc:
    ret1 = cts_set_work_mode(cts_dev, 0); 
    if (ret1) {
        cts_err("Set work_mode to 0");
    }
    return ret;
}
//noise add end
...
static const struct gesture_en_map gesture_en_map_tab[] ={
    ...
    //{VTS_GESTURE_LEFT,      GSTR_EN_MASK_LEFT,      "LEFT"},
   // {VTS_GESTURE_RIGHT,     GSTR_EN_MASK_RIGHT,     "RIGHT"},
};

int cts_set_gesture(struct cts_device *cts_dev, u32 enable)
{
    int i;
    int ret;
    u32 fw_en = 0;
