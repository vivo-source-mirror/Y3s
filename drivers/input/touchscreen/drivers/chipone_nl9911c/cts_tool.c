    ...
    u32 ntests, struct cts_test_param *tests)
{
    ...
    for (i = 0; i < ntests; i++) {
        ...
        if (driver_log_to_file) {
            cts_info("  - Flag: Dump driver log to file with value %d", driver_log_to_file_append);
    ...
    struct cts_upgrade_fw_ioctl_data *ioctl_data)
{
    ...
    if (!!(ioctl_data->flags & CTS_IOCTL_UPGRADE_FW_FLAG_FILE)) {
        ...
        if (ret) {
            cts_err("Upgrade fw from file '%s' failed %d", filepath, ret);
        }
        kfree(filepath);
...
static struct cdev cts_cdev;
static struct class *cts_class;
static int major = 0;
...
int cts_tool_init(struct chipone_ts_data *cts_data)
{
	...
	ret = alloc_chrdev_region(&devid, 0, 1, "cts_tool"); 
	if (ret) {
		cts_err("alloc chrdev error");
		return ret;
	}
	major = MAJOR(devid);                  

	cdev_init(&cts_cdev, &cts_tool_fops);
	ret = cdev_add(&cts_cdev, devid, 1);
	if (ret) {
		cts_err("failed to add cdev");
		return ret;
	}
	cts_class = class_create(THIS_MODULE, "cts_tool");
	device_create(cts_class, NULL, MKDEV(major, 0), NULL, "cts_tool"); /* /dev/cts_tool */
	if (IS_ERR(cts_class)) {
		cts_err("failed to create class");
	}
/*	
...
void cts_tool_deinit(struct chipone_ts_data *data)
{
	...
	device_destroy(cts_class, MKDEV(major, 0));

	class_destroy(cts_class);
