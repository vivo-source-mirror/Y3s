	...
	int x, int y, int wx, int wy, bool large_press, u8 *custom_data, size_t custom_size, ktime_t kt)
{
	...
	for (i = 0; (i < custom_size) && custom_data; i++)
		snprintf(buf + strlen(buf), sizeof(buf) - strlen(buf), "%d ", custom_data[i]);

	if(!test_bit(touch_id, &report->touchbit)) {
		vts_dev_info(vtsdev, "[%d %04d %04d][%04d %04d]down[%d][%d][%d][%d][%lld %lld][%ld][%s]\n", touch_id, x, y, point_info.physical_x, point_info.physical_y, nr_touches, wx, wy, large_press, ktime_to_us(ktime_sub(ktime_get(),kt)), ktime_to_us(kt), report->flags, buf);
#ifdef TOUCH_DATA_COLLECT
		data_collect_increase(TOUCH_VCODE_CLICK_EVENT);
#endif
	} else
		vts_dev_dbg(vtsdev, "[%d %04d %04d][%04d %04d]down[%d][%d][%d][%d][%lld %lld]%s\n", touch_id, x, y, point_info.physical_x, point_info.physical_y, nr_touches, wx, wy, large_press, ktime_to_us(ktime_sub(ktime_get(),kt)), ktime_to_us(kt), buf);

	if (report->on_point_down) {
	
		ret = report->on_point_down(report, touch_id, nr_touches, large_press, &point_info);
	}
	if (ret) {
		vts_dev_info(vtsdev, "down point was ignored![%d][%d][%d][%d][%d][%d][%d]\n", touch_id, nr_touches, x, y, wx, wy, large_press);
		return 0;
	}
...
int vts_report_point_up(struct vts_device *vtsdev, int touch_id, int nr_touches, int x, int y, int wx, int wy, bool large_press, ktime_t kt)
{
	...
	vts_dev_info(vtsdev, "[%d %04d %04d][%04d %04d]up[%d][%d][%d][%lld]\n", touch_id, x, y, point_info.physical_x, point_info.physical_y, nr_touches, wx, wy, ktime_to_us(kt));
	if(!test_bit(touch_id, &report->touchbit)) {
...
*/
static int vts_report_parse_line(u8 *line, size_t size, __kernel_ulong_t *sec, __kernel_ulong_t *usec, u16 *type, u16 *code, u32 *value)
{
	char devicename[64];
	int count;

	memset(devicename, 0, sizeof(devicename));
	count = sscanf(line, "[%8ld.%06ld] %s %hx %hx %x", sec, usec, devicename, type, code, value);
...
static void vts_fill_event(struct input_event *e, __kernel_ulong_t sec, __kernel_ulong_t usec, u16 type, u16 code, u32 value)
{
	e->type = type;
	e->code = code;
	e->value = value;
...
static int vts_report_parse_next_inject_event(struct simulator *s)
{
	...
	while (s->curr_pos < s->size) {
		...
		ret = vts_report_parse_line(line, sizeof(line), &sec, &usec, &type, &code, &value);
		if (ret)
			return ret;

		vts_fill_event(&s->event.events[s->event.nr_events], sec, usec, type, code, value);
		...
		if (time.tv_sec != sec || time.tv_usec!= usec)
			break;
	...
	s->event.time = timeval_to_ktime(time);
	return 0;
}
