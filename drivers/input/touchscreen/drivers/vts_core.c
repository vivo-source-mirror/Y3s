...
static int vts_mode_in_normal_process(struct vts_device *vtsdev, bool mode_change, enum vts_state reason)
{
	...
	__set_state_if_property_support(vtsdev, mode_change, reason, VTS_STA_GAME_MODE, VTS_PROPERTY_GAME_MODE, set_auto_idle, true);
...
static int __vts_state_set(struct vts_device *vtsdev, enum vts_state state, int val, bool async)
{
	...
	vts_event2(VTS_EVENT_STATE_CHANGED, state, val);
...
int vts_register_driver(struct vts_device *vtsdev)
{
	...
	ret = vts_report_init(vtsdev, vts_on_event_down, vts_on_event_up, vts_on_point_down, vts_on_point_up,vts_on_post_key);
	if (ret) {
...
int vts_firmware_version_get(struct vts_device *vtsdev, u64 *version)
{
	...
	vts_report_fw_version(vts_current_module_lcmid(vtsdev->module), *version);
...
static int vts_parse_modeval(const char *format, const char *pre_deli, const char *next_deli, unsigned int *modeval)
{
	int val;
	char *pos;
	char *mode;
	...
	pos = strnstr(str_cmdline, format, strlen(str_cmdline));
	if (pos) {
...
static void vts_parse_cmdline(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(cmdline_mode); i ++) {
		ret = vts_parse_modeval(cmdline_mode[i].mode, cmdline_mode[i].pre_deli, cmdline_mode[i].next_deli, cmdline_mode[i].modeval);
		if (ret) {
