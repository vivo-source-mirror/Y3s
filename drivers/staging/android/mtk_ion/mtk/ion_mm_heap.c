...
#elif (DOMAIN_NUM == 2)
	ION_DUMP(s,
		 "0x%p %5d %8zu %3d %3d %3d %3d %3d %lx(%lx) %lx(%lx) 0x%x, 0x%x, %5d(%5d) %16s %5d %16s 0x%x 0x%x 0x%x 0x%x %s\n",
		 buffer, buffer->heap->id, buffer->size, buffer->kmap_cnt,
		 atomic_read(&buffer->ref.refcount.refs),
		 buffer->handle_count, val,
		 bug_info->mva_cnt,
		 ...
		 bug_info->security,
		 buffer->flags, buffer->pid, bug_info->pid,
		 buffer->task_comm, buffer->current_pid, buffer->current_comm, pdbg->value1,
		 pdbg->value2, pdbg->value3, pdbg->value4,
		 pdbg->dbg_name);
#elif (DOMAIN_NUM == 4)
	ION_DUMP(s,
...
#elif (DOMAIN_NUM == 2)
	ION_DUMP(s,
		 "%18.s %8.s %8.s %4.s %3.s %3.s %3.s %3.s %s %s %3.s %4.s %s %s %4.s %s %4.s %4.s %4.s %4.s %s\n",
		 "buffer", "heap_id", "size", "kmap", "ref", "hdl", "mod",
		 "mva_cnt", "mva(dom0)", "mva(dom1)", "sec", "flag",
		 "pid(alloc_pid)", "comm(client)", "current_pid", "current_comm",
		 "v1", "v2", "v3", "v4",
		 "dbg_name");
#elif (DOMAIN_NUM == 4)
	ION_DUMP(s,
