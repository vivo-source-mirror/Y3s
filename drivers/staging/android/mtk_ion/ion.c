					    ...
					    unsigned long flags)
{
	...
	/* log task pid for debug +by k.zhang */
	{
		...
		buffer->current_pid = current->pid;
		memcpy(buffer->current_comm, current->comm, TASK_COMM_LEN);
	}
...
static int ion_debug_client_show(struct seq_file *s, void *unused)
{
	...
	seq_printf(s, "%16.s %8.s %8.s %16.s %8.s %8.s %8.s %8.s\n",
		   "heap_name", "pid", "current_pid", "current_comm", "size",
	...
	for (n = rb_first(&client->handles); n; n = rb_next(n)) {
		...
		seq_printf(s, "%16.s %3d %3d %16.s %8zu %3d %p %p.\n", buffer->heap->name,
			   client->pid, client->current_pid, client->current_comm, buffer->size,
				     ...
				     const char *name)
{
	...
	client->task = task;
	client->pid = pid;
	client->current_pid = current->pid;
	memcpy(client->current_comm, current->comm, TASK_COMM_LEN);
