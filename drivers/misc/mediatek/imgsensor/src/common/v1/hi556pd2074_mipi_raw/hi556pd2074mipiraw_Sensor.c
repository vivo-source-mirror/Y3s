...
#include "hi556pd2074mipiraw_Sensor.h"

#define PFX "hi556pd2074_camera_sensor"
...
static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = HI556PD2074_SENSOR_ID,
	.checksum_value = 0x55e2a82f,
	.pre = {
		.pclk = 167266666,				//record different mode's pclk
		.linelength  = 2816,				//record different mode's linelength
		.framelength = 1979,			//record different mode's framelength
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[10] = {
	{ 2564, 1920,	2,	  0,   2560, 1920,	 1280, 960,   0, 0, 1280,	960,   0, 0, 1280,	960}, 	// preview
	{ 2564, 1920,	2,	  0,   2560, 1920,   2560, 1920,  0, 0, 2560,	1920,  0, 0, 2560, 1920}, 	// capture
	{ 2564, 1920,	2,	  0,   2560, 1920,	 1280, 960,   0, 0, 1280,	960,   0, 0, 1280,	960}, 	/* video*/
	{ 2576, 1920,	8,  0,   2560, 1920,   640,  480,   0, 0,  640,   480,   0, 0,  640, 480}, 	/* hs_video don't use*/
	{ 2564, 1920,	2,	  0,   2560, 1920,	 1280, 960,   0, 0, 1280,	960,   0, 0, 1280,	960},	// slim video
	{ 2564, 1920,	2,	  0,   2560, 1920,	 1280, 960,   0, 0, 1280,	960,   0, 0, 1280,	960},	// custom1
	{ 2564, 1920,	2,	  0,   2560, 1920,	 1280, 960,   0, 0, 1280,	960,   0, 0, 1280,	960},	// custom2
	{ 2564, 1920,	2,	  0,   2560, 1920,	 1280, 960,   0, 0, 1280,	960,   0, 0, 1280,	960},	// custom3
	{ 2564, 1920,	2,	  0,   2560, 1920,	 1280, 960,   0, 0, 1280,	960,   0, 0, 1280,	960},	// custom4
	{ 2564, 1920,	2,	  0,   2560, 1920,	 1280, 960,   0, 0, 1280,	960,   0, 0, 1280,	960},	// custom5
};

/****vivo caofeixiang add for CameraEM otp errorcode****/
extern int vivo_sub_otp_read_hi556pd2074(void);
int vivo_sub_otp_read_when_power_on_hi556pd2074;
extern otp_error_code_t SUB_OTP_ERROR_CODE_HI556PD2074;
extern unsigned int is_atboot;
MUINT32  sn_inf_sub_hi556pd2074[13];  /*0 flag   1-12 data*/
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	write_cmos_sensor_8(0x0077,reg_gain&0xff);/* max = 0xf0*/
...
static void set_mirror_flip(kal_uint8 image_mirror)
{
   switch (image_mirror) {
        ...
        case IMAGE_NORMAL:
            write_cmos_sensor_8(0x000e,0x00);   /*Gr*/ 
            break;

        case IMAGE_H_MIRROR:
            write_cmos_sensor_8(0x000e,0x01);
            break;

        case IMAGE_V_MIRROR:
            write_cmos_sensor_8(0x000e,0x02);
            break;

        case IMAGE_HV_MIRROR:
            write_cmos_sensor_8(0x000e,0x03);/*Gb*/
            break;
        default:
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x095a, 0x53f0);
	...
	write_cmos_sensor(0x0006, 0x07bb);
...
static void preview_setting(void)
{
	...
	write_cmos_sensor(0x0f30, 0xC155);
	...
	write_cmos_sensor(0x0a14,0x03c0);
	write_cmos_sensor(0x003e,0x0000);
	...
	write_cmos_sensor(0x0958, 0xfa80);
...
//	write_cmos_sensor(0x0a00, 0x0000); // stream off
	if( currefps == 300 )
	{
		...
		write_cmos_sensor(0x002a, 0x0039); 
		write_cmos_sensor(0x0026, 0x003c); 
		write_cmos_sensor(0x002c, 0x07bb); 
		write_cmos_sensor(0x002e, 0x1111); 
		write_cmos_sensor(0x0030, 0x1111); 
		...
		write_cmos_sensor(0x0006, 0x07bb); 
		write_cmos_sensor(0x0a22, 0x0000); 
		write_cmos_sensor(0x0a12, 0x0a00); 
		write_cmos_sensor(0x0a14, 0x0780); 
		write_cmos_sensor(0x003e, 0x0000); 
		write_cmos_sensor(0x0074, 0x0821); 
		write_cmos_sensor(0x0070, 0x0407); 
		write_cmos_sensor(0x0804, 0x0200); 
			...
			do {
				*sensor_id =return_sensor_id() + 1;
				pr_info("i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
				 sensor_idx =return_sensor_id() + 1;
				  pr_info("i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_idx);
				  pr_info("i2c write id: 0x%x,imgsensor_info. sensor id: 0x%x\n", imgsensor.i2c_write_id,imgsensor_info.sensor_id); 
				if (sensor_idx == imgsensor_info.sensor_id) {
				/*vivo hope add for AT cammand start*/
					...
					/*vivo hope add for AT cammand end*/
						...
						sensor_init();
						/*vivo caofeixiang add for CameraEM otp errorcode*/
						LOG_INF("add:start read eeprom ---vivo_sub_otp_read_when_power_on_hi556pd2074 = %d\n", vivo_sub_otp_read_when_power_on_hi556pd2074);
						vivo_sub_otp_read_when_power_on_hi556pd2074 = vivo_sub_otp_read_hi556pd2074();
						LOG_INF("add:end read eeprom ---vivo_sub_otp_read_when_power_on_hi556pd2074 = %d,SUB_OTP_ERROR_CODE_HI556PD2074 =%d\n", vivo_sub_otp_read_when_power_on_hi556pd2074,SUB_OTP_ERROR_CODE_HI556PD2074);
						/*vivo caofeixiang add end*/
						return ERROR_NONE;
                   }
				   else{
			...
			UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	break;
        case SENSOR_FEATURE_GET_CUSTOM_INFO:
		    printk("SENSOR_FEATURE_GET_CUSTOM_INFO information type:%lld  SUB_OTP_ERROR_CODE_HI556PD2074:%d \n", *feature_data,SUB_OTP_ERROR_CODE_HI556PD2074);
			switch (*feature_data) {
				case 0:    //info type: otp state
				    *(MUINT32 *)(uintptr_t)(*(feature_data+1)) = SUB_OTP_ERROR_CODE_HI556PD2074;//otp_state
					memcpy(feature_data + 2, sn_inf_sub_hi556pd2074, sizeof(MUINT32)*13);
					break;
			}
			break;
