 ...
 *============================================================================
 ****************************************************************************/
#define PFX "S5K3L6XXPD2074v2_camera_sensor"
...
#include "s5k3l6xxpd2074v2mipiraw_Sensor.h"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = S5K3L6XXPD2074V2_SENSOR_ID,
...
extern int s5k3l6xx_vivo_otp_read(void);
extern bool vivo_read_eeprom(kal_uint16 addr,  BYTE *data);
static int vivo_otp_read_when_power_on = 0;
extern otp_error_code_t S5K3L6XX_OTP_ERROR_CODE;
extern MUINT32  sn_inf_main_s5k3l6xx[13];  /*0 flag   1-12 data*/ 
extern unsigned int is_atboot;
extern unsigned  int s5k3l6xx_flag;
/*vivo hope add end*/
...
0x0340, 0x0CBC,
0x38C4, 0x0006,
0x38D8, 0x0018,  //samsung modify 20210225 old:0x001A
0x38DA, 0x0006,  //samsung modify 20210225 old:0x0007
0x38DC, 0x0007,
0x38C2, 0x0006,
0x38C0, 0x0007,  //samsung modify 20210225 old:0x0008
0x38D6, 0x0006,
0x38D4, 0x0005,
0x38B0, 0x0009,  //samsung modify 20210225 old:0x000A
0x3932, 0x1800,  //samsung modify 20210225 old:0x1000
0x3938, 0x000C,
0x0820, 0x02E0,  //samsung modify 20210225 old:0x02DF
0x380C, 0x0049,
0x3064, 0xFFCF,
0x309C, 0x0640,
0x3090, 0x8000,
...
0x32B6, 0x0003,
0x32B8, 0x0003,
0x3C34, 0x0048,  //samsung modify 20210225 old:0x0008
0x3C36, 0x3000,
0x3C38, 0x0024,  //samsung modify 20210225 old:0x0020
0x393E, 0x4000,
0x303A, 0x0204,
0x3034, 0x4B01,
0x3036, 0x0029,
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		spin_unlock(&imgsensor_drv_lock);
		if(vivo_read_eeprom(1,&temp_val)){
			if(temp_val!=0x02){  //0x05:qtech  0x02:truly
				*sensor_id = 0xFFFFFFFF;
				return ERROR_SENSOR_CONNECT_FAIL;
			}
		}
		...
		do {
				*sensor_id = return_sensor_id() + 3;
				...
				vivo_otp_read_when_power_on = s5k3l6xx_vivo_otp_read();
			
				pr_debug("add:end read eeprom - when_power_on = %d\n",vivo_otp_read_when_power_on);
			   	return ERROR_NONE;
				/*vivo hope add end*/
				if (s5k3l6xx_flag == 1){
					pr_debug("i2c write id: 0x%x, sensor id: 0x%x, s5k3l6xx_flag = %d\n", imgsensor.i2c_write_id,*sensor_id, s5k3l6xx_flag);
					return ERROR_NONE;
				}
			}
			pr_debug("Read sensor id fail, id: 0x%x, s5k3l6xx_flag =%d\n",
				imgsensor.i2c_write_id, s5k3l6xx_flag);
			retry--;
		} while (retry > 0);
		i++;
		retry = 2;
	}
	if (*sensor_id != imgsensor_info.sensor_id || s5k3l6xx_flag != 1) {
...
static kal_uint32 close(void)
{
	...
	/*No Need to implement this function*/
	streaming_control(KAL_FALSE);  //[B210406-2473] lishuo add 20210406 FIXME
	return ERROR_NONE;
}	/*	close  */
				 ...
				 UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_CUSTOM_INFO:
		pr_debug("SENSOR_FEATURE_GET_CUSTOM_INFO information type:%lld  S5K3L6XX_OTP_ERROR_CODE:%d \n", *feature_data,S5K3L6XX_OTP_ERROR_CODE);
			switch (*feature_data) {
			...
			*/
					*(MUINT32 *)(uintptr_t)(*(feature_data+1)) = S5K3L6XX_OTP_ERROR_CODE;//otp_state
					...
					#if 0
						for (i = 0 ; i<12 ; i++ ){
					pr_info("sn_inf_main_s5k3l6xx[%d]= 0x%x\n", i, sn_inf_main_s5k3l6xx[i]);
						}
