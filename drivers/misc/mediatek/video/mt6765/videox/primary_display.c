...
int primary_display_get_debug_state(char *stringbuf, int buf_len)
{
	...
	if (buf_len - len > 1)
		len += scnprintf(stringbuf + len, buf_len - len,
		"|State=%s\tlcm_fps=%u\tmax_layer=%d\tmode:%s\tvsync_drop=%d\n",
...
static int init_decouple_buffers(void)
{
	...
	if (disp_helper_get_option(DISP_OPT_GMO_OPTIMIZE)) {
		...
		for (i = 0; i < DISP_INTERNAL_BUFFER_COUNT; i++) {
			...
			mva = (unsigned int)primary_display_get_frame_buffer_mva_address();
			if ((MAX_INT - mva) >= i * buffer_size)
				pgc->dc_buf[i] = (unsigned int)(i * buffer_size) + mva;
			else
				DISPERR("%s,frame buf:%d out of range,%d\n", __func__, i);
		}
	}
