...
static void smi_disable_throttle(){

	s32 i;
	u32 time = 5;
	u32 smi_debug_misc_time[4] = {0}, smi_debug_m0_time = 0, smi_debug_m1_time = 0;

	/* check underflow & master port outs*/
	for (i = 0; i < time; i++) {
		pr_info("[SMI]smi_disable_throttle check start\n");
		if (readl(smi_dev[SMI_LARB_NUM]->base + SMI_DEBUG_MISC) & 0x10)
			smi_debug_misc_time[0] ++;
		if (readl(smi_dev[SMI_LARB_NUM]->base + SMI_DEBUG_MISC) & 0x20)
			smi_debug_misc_time[1] ++;
		if (readl(smi_dev[SMI_LARB_NUM]->base + SMI_DEBUG_MISC) & 0x100)
			smi_debug_misc_time[2] ++;
		if (readl(smi_dev[SMI_LARB_NUM]->base + SMI_DEBUG_MISC) & 0x200)
			smi_debug_misc_time[3] ++;

		if (readl(smi_dev[SMI_LARB_NUM]->base + SMI_DEBUG_M0) & 0x3ff000)
			smi_debug_m0_time ++;
		if (readl(smi_dev[SMI_LARB_NUM]->base + SMI_DEBUG_M1) & 0x3ff000)
			smi_debug_m1_time ++;
	}
	if ((smi_debug_misc_time[0] == time || smi_debug_misc_time[1] == time
		|| smi_debug_misc_time[2] == time || smi_debug_misc_time[3] == time)
		&& smi_debug_m0_time == time && smi_debug_m1_time == time) {
		pr_info("[SMI]smi start disable throttle after checking\n");
		writel(readl(smi_dev[SMI_LARB_NUM]->base + SMI_L1LEN) & 0xFFFFFFF7,
			smi_dev[SMI_LARB_NUM]->base + SMI_L1LEN);
		pr_info("[SMI]smi finish disable throttle after checking, offset:%#x value:%#x\n",
			SMI_L1LEN, readl(smi_dev[SMI_LARB_NUM]->base + SMI_L1LEN));
	} else {
		pr_info("[SMI]smi always disable throttle\n");
		writel(readl(smi_dev[SMI_LARB_NUM]->base + SMI_L1LEN) & 0xFFFFFFF7,
			smi_dev[SMI_LARB_NUM]->base + SMI_L1LEN);
		pr_info("[SMI]smi finish always disable throttle, offset:%#x value:%#x\n",
			SMI_L1LEN, readl(smi_dev[SMI_LARB_NUM]->base + SMI_L1LEN));
	}
...
s32 smi_debug_bus_hang_detect_disp(const bool gce, const char *user)
{
	...
	pr_info("[SMI]start smi_disable_throttle\n");
	smi_disable_throttle();
