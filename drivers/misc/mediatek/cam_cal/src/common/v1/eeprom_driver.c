...
extern int vivo_sub_otp_read_when_power_on_s5k4h7yxpd1901;
extern unsigned char vivo_sub_otp_data_s5k4h7yxpd1901[1886];
extern unsigned char  sn_inf_sub_s5k4h7yxpd1901[];
extern int vivo_sub_otp_read_when_power_on_s5k4h7yxpd1901v1;
extern unsigned char vivo_sub_otp_data_s5k4h7yxpd1901v1[1886];
extern unsigned char  sn_inf_sub_s5k4h7yxpd1901v1[];
#endif
...
#endif
{
...
#endif
		pr_info("CAM_CALIOC_S_WRITE End!\n");
		break;
	...
	case CAM_CALIOC_G_READ:
...
#if defined(CONFIG_MTK_CAM_PD1901)
		pr_info("CONFIG_MTK_CAM_PD1901\n");
		/* hope add for s5k4h7yx*/
		if (ptempbuf->sensorID == 0x487b && vivo_sub_otp_read_when_power_on_s5k4h7yxpd1901 == 1) {
			if (ptempbuf->u4Length == 4 && ptempbuf->u4Offset == 0x0000) {/*read mtk calibration id*/
				pr_info("read S5K4H7YX sensor_otp data start copy,length 4!\n");
				memcpy(pu1Params, &calibration_versiongned[0], 4);
				pr_info("read S5K4H7YX sensor_otp Memcpy end!\n");
				/*for (i = 0; i < 4; i++) {
				pr_info("read S5K4H7YX pu1Params[%d]:%d\n", i, pu1Params[i]);
				}*/
			} else if (ptempbuf->u4Length == 12 && ptempbuf->u4Offset == 0x0006) { /*read s5k4h7yx awb data*/
				pr_info("read S5K4H7YX awb data start copy,length 12 !\n");
				memcpy(pu1Params, &vivo_sub_otp_data_s5k4h7yxpd1901[6], 12);
				pr_info("read S5K4H7YX awb Memcpy end!\n");
			} else if (ptempbuf->u4Length == 1868 && ptempbuf->u4Offset == 0x0012) { /*read lsc data*/
				pr_info("read S5K4H7YX lsc data start copy,length 1868!\n");
				memcpy(pu1Params, &vivo_sub_otp_data_s5k4h7yxpd1901[18], 1868);
				pr_info("read S5K4H7YX lsc Memcpy end!\n");
			} else {
				i4RetValue = 0;
			}
			...
			if (_IOC_READ & _IOC_DIR(a_u4Command)) {
				...
				if (copy_to_user((u8 __user *) ptempbuf->pu1Params, (u8 *)pu1Params, ptempbuf->u4Length)) {
					kfree(pBuff);
					kfree(pu1Params);
					pr_info("ioctl copy to user failed\n");
					return -EFAULT;
				}
		...
		else if (ptempbuf->sensorID == 0x487b && vivo_sub_otp_read_when_power_on_s5k4h7yxpd1901 != 1) {
			pr_info("read S5K4H7YX kernel otp read failed!,vivo_sub_otp_read_when_power_on_s5k4h7yxpd1901 = %d\n", vivo_sub_otp_read_when_power_on_s5k4h7yxpd1901);
			//[B210312-1533] lishuo begin
			kfree(pBuff);	 
			kfree(pu1Params);
			//[B210312-1533] lishuo end
		...
		/*hope add for s5k4h7yx*/
			/* hope add for s5k4h7yx*/
		if (ptempbuf->sensorID == 0x487c && vivo_sub_otp_read_when_power_on_s5k4h7yxpd1901v1 == 1) {
			if (ptempbuf->u4Length == 4 && ptempbuf->u4Offset == 0x0000) {/*read mtk calibration id*/
				pr_info("read S5K4H7YXV1 sensor_otp data start copy,length 4!\n");
				memcpy(pu1Params, &calibration_versiongned[0], 4);
				pr_info("read S5K4H7YXV1 sensor_otp Memcpy end!\n");
				/*for (i = 0; i < 4; i++) {
				pr_info("read S5K4H7YX pu1Params[%d]:%d\n", i, pu1Params[i]);
				}*/
			} else if (ptempbuf->u4Length == 12 && ptempbuf->u4Offset == 0x0006) { /*read s5k4h7yx awb data*/
				pr_info("read S5K4H7YXV1 awb data start copy,length 12 !\n");
				memcpy(pu1Params, &vivo_sub_otp_data_s5k4h7yxpd1901v1[6], 12);
				pr_info("read S5K4H7YXV1 awb Memcpy end!\n");
			} else if (ptempbuf->u4Length == 1868 && ptempbuf->u4Offset == 0x0012) { /*read lsc data*/
				pr_info("read S5K4H7YXV1 lsc data start copy,length 1868!\n");
				memcpy(pu1Params, &vivo_sub_otp_data_s5k4h7yxpd1901v1[18], 1868);
				pr_info("read S5K4H7YXV1 lsc Memcpy end!\n");
			} else {
				i4RetValue = 0;
			}
			i4RetValue = ptempbuf->u4Length;
		...
		else if (ptempbuf->sensorID == 0x487c && vivo_sub_otp_read_when_power_on_s5k4h7yxpd1901v1 != 1) {
			pr_info("read S5K4H7YXV1 kernel otp read failed!,vivo_sub_otp_read_when_power_on_s5k4h7yxpd1901v1 = %d\n", vivo_sub_otp_read_when_power_on_s5k4h7yxpd1901v1);
			//[B210312-1533] lishuo begin
			kfree(pBuff);    
			kfree(pu1Params);
			//[B210312-1533] lishuo end
		...
		if (pcmdInf != NULL) {
			if (pcmdInf->readCMDFunc != NULL)
				...
				pr_info("pcmdInf->readCMDFunc == NULL\n");
				kfree(pBuff);
				kfree(pu1Params);
				return -EFAULT;
			}
