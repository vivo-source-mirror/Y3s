					 ...
					 unsigned long arg)
{
	...
	case ALSPS_IOCTL_GET_ALS_REG_DATA:
		...
		if (alsps_factory.fops != NULL && alsps_factory.fops->do_vivo_commands != NULL) {
			cmd_args[0] = SENSOR_COMMAND_LIGHT_READ_REG_DATA;
			err = alsps_factory.fops->do_vivo_commands(ID_LIGHT, cmd_args, sizeof(cmd_args)/sizeof(cmd_args[0]));
			if (err < 0) {
	...
	case ALSPS_IOCTL_CHECK_PS_INT:
		if (alsps_factory.fops != NULL && alsps_factory.fops->do_vivo_commands != NULL) {
			...
			err = alsps_factory.fops->do_vivo_commands(ID_PROXIMITY, cmd_args, sizeof(cmd_args)/sizeof(cmd_args[0]));
			if (err < 0) {
	...
	case ALSPS_IOCTL_SET_ENG_MODE:
		...
		if (alsps_factory.fops != NULL && alsps_factory.fops->do_vivo_commands != NULL) {
			...
			err = alsps_factory.fops->do_vivo_commands(ID_PROXIMITY, cmd_args, ARRAY_SIZE(cmd_args));
			if (err < 0) {
...
#ifndef CONFIG_VIVO_ALS_LOCATE_KERNEL
	alsps_factory.fops = dev->fops;
#else
	alsps_factory.fops = &factory_fops;
#endif
...
#ifdef CONFIG_VIVO_ALS_LOCATE_KERNEL
int als_factory_operation_register(struct alsps_factory_public *dev)
{
	int err = 0;

	if (!dev || !dev->fops)
		return -1;
	factory_fops.als_enable_sensor = dev->fops->als_enable_sensor;
	factory_fops.als_get_data = dev->fops->als_get_data;
	factory_fops.als_get_raw_data = dev->fops->als_get_raw_data;
	factory_fops.als_enable_calibration = dev->fops->als_enable_calibration;
	factory_fops.als_clear_cali = dev->fops->als_clear_cali;
	factory_fops.als_set_cali = dev->fops->als_set_cali;
	factory_fops.als_get_cali = dev->fops->als_get_cali;
	factory_fops.als_local_commands = dev->fops->als_local_commands;
	return err;
}

/// TODO: CMP to check
int ps_factory_operation_register(struct alsps_factory_public *dev)
{
	int err = 0;

	if (!dev || !dev->fops)
		return -1;
	factory_fops.ps_enable_sensor = dev->fops->ps_enable_sensor;
	factory_fops.ps_get_data = dev->fops->ps_get_data;
	factory_fops.ps_get_raw_data = dev->fops->ps_get_raw_data;
	factory_fops.ps_enable_calibration = dev->fops->ps_enable_calibration;
	factory_fops.ps_clear_cali = dev->fops->ps_clear_cali;
	factory_fops.ps_set_cali = dev->fops->ps_set_cali;
	factory_fops.ps_get_cali = dev->fops->ps_get_cali;
	factory_fops.ps_set_threshold = dev->fops->ps_set_threshold;
	factory_fops.ps_get_threshold = dev->fops->ps_get_threshold;
	factory_fops.do_vivo_commands = dev->fops->do_vivo_commands;
	return err;
}
#endif
