...
int als_extension_data_report_t(int *value, int status, int64_t time_stamp)
{
	int err = 0;
	struct alsps_context *cxt = NULL;
	struct sensor_event event;
	...
	event.word[6] = (int16_t)((value[6] & 0xFFFF0000) >> 16);
	event.word[7] = (int16_t)(value[6] & 0x0000FFFF);
	event.word[8] = (((int16_t)((value[7] & 0xFFFF0000) >> 16))*100 + (value[7] & 0x01));
	event.status = status;
	/*pr_err("%s als_data: %d %d %d %d %d id:%d angle:%d %d %d motion:%d\n", __func__,
		event.word[0], event.word[1], event.word[2], event.word[3], event.word[4],
		event.word[5], event.word[6]/100, event.word[7]/100, event.word[8]/100/100, event.word[8]%100);*/
	err = sensor_input_event(cxt->als_mdev.minor, &event);
